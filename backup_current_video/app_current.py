...
- Video Forensics: 20-frame equidistant extraction + EfficientNet-B3 frame classification
=============================================================================
"""
...
VIDEO_MODEL_PATH = os.path.join(base_dir, 'models', 'video_frame_model.keras')
...
try:
    ...
    print(f"[SYSTEM] Loaded EfficientNet-B3 video frame model from {VIDEO_MODEL_PATH}")
except Exception as e:
    print(f"[ERROR] Could not load video frame model: {e}")
    video_model = None
...
def predict_video():
    """
    Analyzes an uploaded video across 20 equidistant frames
    using the EfficientNet-B3 frame classification model
    ...
    try:
        ...
        # 2. Frame-level prediction using EfficientNet-B3 video frame model
        frames_batch = seq_tensor[0]  # (20, 300, 300, 3)

        frame_predictions = video_model.predict(
            frames_batch,
            verbose=0
        ).reshape(-1)
        ...
        for i, prob in enumerate(frame_predictions, start=1):
            print(f"Frame {i:02d}: FAKE = {prob * 100:.2f}%")

        print(f"\nMean FAKE probability: {np.mean(frame_predictions) * 100:.2f}%")
        print(f"Max FAKE probability: {np.max(frame_predictions) * 100:.2f}%")
        print(
            f"Top-5 mean FAKE probability: "
            f"{np.mean(np.sort(frame_predictions)[-5:]) * 100:.2f}%"
        )

        print("=============================================\n")
        # 3. Aggregate frame-level predictions
        raw_pred = float(np.mean(frame_predictions))

        fake_prob = float(np.clip(raw_pred, 0.01, 0.99))
        real_prob = float(1.0 - fake_prob)
        ...
        confidence = (
            round(fake_prob * 100, 1)
            if is_fake
            else round(real_prob * 100, 1)
        )
        ...
        result_payload = {
            ...
            'fake_conf': round(fake_prob * 100, 1),
            'real_conf': round(real_prob * 100, 1),
            'disclaimer': "Probabilistic assessment based on frame-level facial features and aggregated video predictions."
        }
        ...
        return render_template(
            ...
            video_analysis_method="EfficientNet-B3 Frame Analysis"
        )
