...
### B. Video Frame Classification & Probability Aggregation
...
│   └── video_model.keras          # Trained video-frame model
...
│   └── train_video_model.py       # video-frame model training & evaluation
...
### Train Video Frame Model (EfficientNet-B3)
...
[SYSTEM] Loaded EfficientNet-B3 video-frame model from models/video_frame_model.keras
...
   - Select the **"Video Detection (frame-level EfficientNet-B3 approach)"** tab.

   - Drag and drop or browse a video file (`.mp4`, `.avi`, `.mov`).

   - Click **"Analyze Authenticity"**.

   - The system samples video frames, processes them with the video-frame classifier, and aggregates the frame-level predictions.
...
The implemented video-frame pipeline was evaluated on 518 source-video test sequences:
...
The test set contained 340 fake and 178 real source-video sequences.

These results correspond to EfficientNet-B3 frame-level classification with probability aggregation and are not CNN-LSTM results.

### Academic Disclaimer

In accordance with ethical data science guidelines, predictions generated by this system represent probabilistic likelihood scores based on learned visual representations. No automated computer vision system can guarantee 100% deterministic certainty.